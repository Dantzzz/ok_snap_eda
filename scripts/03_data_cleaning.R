...
### Review
paste0(min(df_clean$Date), " - ", max(df_clean$Date)) # Jan12 - Jan19
colnames(df)
class(df$Date)
class(df$FIPS)
...
##  - NPA participation vs PA participation
##  - Participation vs issuance; are there instances in which particpation > issuance? Why?
##  - For BigQuery: Is Oklahoma anomolous vis-a-vis Program == 'EBT'?
